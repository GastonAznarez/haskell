ano :: (String, Int, Int, String) -> Int
ano (x, y, z, t) = y

nombre :: (String, Int, Int, String) -> String
nombre (x, y, z, t) = x

direc :: (String, Int, Int, String) -> String
direc (x, y, z, t) = t
...
cantPelis :: [Pelicula] -> Int
cantPelis [] = 0
cantPelis x = length x

estrenos :: [Pelicula] -> Int -> [Pelicula]
estrenos  [] comp = []
estrenos (x:xs) comp | ano x == comp = x : estrenos xs comp 
                     | ano x /= comp = estrenos xs comp

estrenosNom :: [Pelicula] -> Int -> [String]
estrenosNom  [] comp = []
estrenosNom (x:xs) comp | ano x == comp = nombre x : estrenosNom xs comp 
                        | ano x /= comp = estrenosNom xs comp
  

estrenosNomS :: [Pelicula] -> Int -> String
estrenosNomS  [] comp = []
estrenosNomS (x:xs) comp | ano x == comp = nombre x ++ " \n " ++ estrenosNomS xs comp 
                         | ano x /= comp = estrenosNomS xs comp





directores :: [Pelicula] -> [String]
directores [] = []
directores (x:xs) = (direc x):(directores xs)


pertenece :: String -> [String] -> Bool
pertenece s [] = False
pertenece s (x:xs) | s == x = True
                   | not (s == x) = pertenece s xs

sacarRep :: [String] -> [String]
sacarRep [] = []
sacarRep (x:xs) | pertenece x xs = sacarRep xs
                | not(pertenece x xs) = x:(sacarRep xs)


rep :: [String] -> [String]
rep [] = []
rep (x:xs) | not(pertenece x xs) = rep xs
                | pertenece x xs = x:(rep xs)

numRep :: String -> [String] -> Int
numRep s [] = 0
numRep s (x:xs) | s == x = 1+ (numRep s xs)
                | not (s == x) = numRep s xs


directoresNum :: [String] -> [(String, Int)]
directoresNum [] = []
directoresNum (x:xs) = (x, (numRep x (directores peliculas))):directoresNum xs

repeticiones = directoresNum (sacarRep (directores peliculas))
